...
public:
    ...
    std::unique_ptr<ModuleAST> parseModule() {
        ...
        while (true) {
            ...
            default:
                return parseError<ModuleAST>("struct or def", "top of module");
            case tok_def:
...
private:
    Lexer&                            lexer;
    std::map<std::string, StructAST*> structList;
    ...
    std::unique_ptr<ExprAST> parseStructLiteralExpr() {
        ...
        while (lexer.getCurToken() != tok_bracket_close) {
            auto value = parseExpression();
            values.push_back(std::move(value));
            if (lexer.getCurToken() == Token(','))
                lexer.getNextToken();
            else if (lexer.getCurToken() == tok_identifier)
                return parseError<ExprAST>(",", "struct literal expression");
        }
    ...
    /// an identifier and an optional type (shape specification) before the
    /// initializer.
    ...
    std::unique_ptr<VarDeclExprAST> parseVarDeclaration(bool requiresInitializer) {
        ...
        type->typeName = "tensor";

        if (requiresInitializer == false && lexer.getCurToken() != '=') {
            return std::make_unique<VarDeclExprAST>(
                std::move(loc), std::move(id), std::move(*type), nullptr);
        }
    ...
    // Parse a struct variable declaration, it starts with a `struct name` followed by an
    // identifier and an expression list started with `{` ended with `}`.
    std::unique_ptr<VarDeclExprAST> parseTypedDeclaration(bool requiresInitializer) {
        std::string typeName(lexer.getId());
        VarType     varType{ typeName };
        auto        loc = lexer.getLastLocation();
        lexer.consume(tok_identifier);
        if (lexer.getCurToken() != tok_identifier) {
            return parseError<VarDeclExprAST>("variable name", "in variable declaration");
        }
        std::string varName(lexer.getId());
        lexer.consume(tok_identifier);

        if (requiresInitializer == false && lexer.getCurToken() != '=') {
            return std::make_unique<VarDeclExprAST>(
                loc, std::move(varName), std::move(varType), nullptr);
        }
        if (lexer.getCurToken() != '=')
        ...
        auto expr = parseExpression();
        return std::make_unique<VarDeclExprAST>(
            std::move(loc), std::move(varName), std::move(varType), std::move(expr));
    }


    std::unique_ptr<VarDeclExprAST> parseDeclaration(bool requiresInitializer = true) {
        if (lexer.getCurToken() == tok_var) {
            return parseVarDeclaration(requiresInitializer);
        }
        else if (lexer.getCurToken() == tok_identifier) {
            return parseTypedDeclaration(requiresInitializer);
        }
    ...
    std::unique_ptr<ExprASTList> parseBlock() {
        ...
        while (lexer.getCurToken() != '}' && lexer.getCurToken() != tok_eof) {
            if (lexer.getCurToken() == tok_var) {
                ...
                if (structList.find(id) != structList.end()) {
                    // struct declare
                    auto expr = parseDeclaration();
                    if (!expr)
                        return nullptr;
    ...
    std::unique_ptr<PrototypeAST> parsePrototype() {
        ...
        if (lexer.getCurToken() != ')') {
            do {
                std::string typeName = std::string(lexer.getId());
                VarType     type({ std::string(lexer.getId()) });
                lexer.consume(tok_identifier);

                auto name = lexer.getId();
                auto loc = lexer.getLastLocation();
                lexer.consume(tok_identifier);

                auto decl = std::make_unique<VarDeclExprAST>(
                    std::move(loc), std::move(name), type, nullptr);
    ...
    std::unique_ptr<RecordAST> parseDefinition() {
        ...
        if (auto block = parseBlock())
            return std::make_unique<FunctionAST>(
                proto->loc(), std::move(proto), std::move(block));
        return nullptr;
    }

    /// Parse a struct defination, we except a struct initialized with the `struct`
    /// keyword, followed by a block of variable declare.
    std::unique_ptr<StructAST> parseStruct() {
        auto loc = lexer.getLastLocation();
        lexer.consume(tok_struct);
        if (lexer.getCurToken() != tok_identifier) {
            return parseError<StructAST>("name", "in struct defination");
        }

        std::string structName(lexer.getId());
        lexer.consume(tok_identifier);

        if (lexer.getCurToken() != '{') {
            return parseError<StructAST>("{", "in struct defination");
        }
        lexer.consume(Token('{'));

        std::vector<std::unique_ptr<VarDeclExprAST>> memberDecls;
        do {
            auto decl = parseDeclaration(false);
            if (!decl) {
                return nullptr;
            }
            memberDecls.push_back(std::move(decl));

            if (lexer.getCurToken() == Token(';'))
                lexer.consume(Token(';'));
            else
                return parseError<StructAST>("; or }", "in struct defination");
        ...
        auto structAST = std::make_unique<StructAST>(
            std::move(loc), std::move(structName), std::move(memberDecls));

        structList.insert({ std::string(structAST->getTypeName()), structAST.get() });
        return structAST;
    }
