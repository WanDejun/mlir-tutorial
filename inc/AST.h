...
public:
    ...
    RecordAST(RecordASTKind kind, Location location)
        : kind(kind), location(std::move(location)) {}
...
public:
    VarDeclExprAST(
        ...
        VarType                  type,
        std::unique_ptr<ExprAST> initVal)
        : ExprAST(Expr_VarDecl, std::move(loc)), name(name), type(std::move(type)),
...
/// This class represents a literal struct value.
...
public:
    ...
    llvm::ArrayRef<std::unique_ptr<ExprAST>> getValues() {
        return llvm::ArrayRef<std::unique_ptr<ExprAST>>(values);
    }

    static bool classof(const ExprAST* AST) {
        return AST->getKind() == Expr_StructLiteral;
    }
};
...
public:
    FunctionAST(
        ...
        std::unique_ptr<PrototypeAST> proto,
        std::unique_ptr<ExprASTList>  body)
        : RecordAST(Record_Function, loc), proto(std::move(proto)),
          body(std::move(body)) {}
    ...
    static bool classof(const RecordAST* c) {
        return c->getKind() == Record_Function;
    }
};


// This class represents an struct defination. Like:
//  struct my_type {
//      var a;
//      var b;
//  };
class StructAST : public RecordAST {
    std::string typeName;
    // store the member's names in the struct.
    std::vector<std::unique_ptr<VarDeclExprAST>> members;
...
public:
    StructAST(
        ...
        std::string                                  typeName,
        std::vector<std::unique_ptr<VarDeclExprAST>> members)
        : RecordAST(Record_Struct, std::move(loc)), typeName(typeName),
    ...
    llvm::ArrayRef<std::unique_ptr<VarDeclExprAST>> getMembers() {
        return members;
    }

    static bool classof(const RecordAST* c) {
        return c->getKind() == Record_Struct;
    }
};
